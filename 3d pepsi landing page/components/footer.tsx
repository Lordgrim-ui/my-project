...
export function Footer() {
  ...
  const footerLinks = [
    {
      title: "Products",
      links: ["Pepsi Classic", "Pepsi Zero Sugar", "Pepsi Wild Cherry", "All Variants"],
    },
    {
      title: "Quick Links",
      links: ["Home", "Products", "Taste Stories", "Where to Buy"],
    },
    {
      title: "Company",
      links: ["About Pepsi", "Careers", "Press", "Contact Us"],
    },
    {
      title: "Legal",
    ...
    <footer ref={footerRef} id="careers" className="relative bg-[#121212] pt-16 pb-6 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="flex flex-col sm:flex-row gap-3">
            <motion.div className="flex-1 relative" whileFocus={{ scale: 1.02 }}>
              <motion.input
                ...
                className="w-full bg-white/5 border-2 border-white/20 rounded-xl px-4 py-3 text-white placeholder:text-white/40 font-mono text-sm focus:outline-none focus:border-[#004B87] transition-all duration-300"
                whileFocus={{ borderColor: "#004B87" }}
              />
              <motion.div
                className="absolute inset-0 rounded-xl pointer-events-none"
                animate={email.length > 0 ? { boxShadow: "0 0 20px rgba(0,75,135,0.2)" } : { boxShadow: "none" }}
              />
            </motion.div>
            <motion.button
              className="bg-[#004B87] text-white px-6 py-3 rounded-xl font-bold text-sm tracking-wide whitespace-nowrap relative overflow-hidden"
          ...
          >
            Join millions worldwide. No spam, just refreshment.
          </motion.p>
        </motion.div>
        ...
        >
          <p className="text-white/60 font-mono text-xs max-w-xl mx-auto leading-relaxed">
            Pepsi is the iconic cola beverage enjoyed by billions worldwide. Bold, refreshing, and perfectly balanced—the taste that generations have loved since 1893.
          </p>
        </motion.div>
        ...
        >
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 400, damping: 17 }}
          >
            <span className="text-xl font-black text-white">PEPSI</span>
          </motion.div>

          <p className="text-white/40 font-mono text-xs">© 2026 FOLARANMI ABDULBASIT. All rights reserved.</p>
